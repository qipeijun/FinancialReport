...
from scripts.utils.providers import GeminiProvider
from scripts.utils.print_utils import print_error


def parse_args() -> argparse.Namespace:
    """解析命令行参数"""
    parser = argparse.ArgumentParser(description='生成带实时数据验证的AI财经分析报告（Gemini 3.0）')
...
def load_api_key(args: argparse.Namespace) -> str:
    ...
    if config_path.exists():
        try:
            ...
            api_key = (cfg.get('api_keys') or {}).get('gemini') or (cfg.get('gemini') or {}).get('api_key')
            if api_key:
                return api_key
        except Exception:
            pass
...
def main():
    ...
    try:
        result = generator.generate(
            ...
            prompt_version='pro_v2',  # 使用增强版提示词
